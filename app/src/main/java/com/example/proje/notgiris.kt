...
class notgiris : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_notgiris)
        lateinit var user:String
        user = intent.getStringExtra("Name").toString()
        val button = findViewById<Button>(R.id.button)
        val sname = findViewById<EditText>(R.id.ogrenci)
    ...
    private fun note_save(uname: String,sname: String,vize: String,final: String) {
        ...
        val jsonapi: JsonObjectRequest = object : JsonObjectRequest(
            ...
            Response.Listener { response ->
                Toast.makeText( this,response.getString("name").toString() ,Toast.LENGTH_SHORT).show()
            },
